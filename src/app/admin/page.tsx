...
import type { AnyEntry, Tag, UserSubmission, NewEntrySuggestionData, EditEntrySuggestionData, ExiconEntry, UserSubmissionBase, Alias } from '@/lib/types';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function AdminPage() {
  ...
  const handleAddNewEntry = () => {
    setEditingEntry({
      type: 'lexicon', // Default to lexicon, can be changed in form
      id: '', 
      name: '',
      description: '',
      aliases: [],
      // linkedDescriptionHtml will be generated server-side if needed or handled by display
    });
  ...
  const handleEntryFormSubmit = async (data: AnyEntry) => {
    try {
      if (editingEntry?.id && editingEntry.id !== '') { // Check if editingEntry and its id are valid
        ...
        // For creates, the ID might be generated by the form or backend
        // The createEntryInDatabase action in admin/actions.ts should handle Omit<AnyEntry, 'id'>
        const dataToCreate = data as Omit<AnyEntry, 'id' | 'linkedDescriptionHtml'> & { id?: string };
        // Remove temporary ID if it exists and starts with type prefix, let DB handle it
  ...
  const handleViewSubmission = async (submission: UserSubmissionBase<any>) => {
    ...
    if (submission.submissionType === 'edit') {
      ...
      try {
        ...
        console.log("Original Entry Aliases Structure:", originalEntry?.aliases); // Log original aliases
      } catch (error) {
      ...
      // Add console.log here to inspect the suggested aliases data
      console.log(
        "Suggested Aliases Data Structure:", (submission.data as EditEntrySuggestionData).changes.aliases
      );
    }
    ...
    <PageContainer>
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <Dialog open={isEntryFormOpen} onOpenChange={(isOpen) => {
          ...
          if (!isOpen) {
            setEditingEntry(undefined); // Clear editing state when dialog closes
          }
        }}>
        ...
        <DialogContent className="sm:max-w-[725px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="py-4 space-y-4">
              ...
              {viewingSubmission.submissionType === 'edit' && originalEntryForEditView && (
                <>
                  ...
                  {viewingSubmission.data.entryType === 'exicon' && originalEntryForEditView.type === 'exicon' && (
                    <>
                      {(viewingSubmission.data as EditEntrySuggestionData).changes.videoLink !== undefined && (
                        <div>
                            ...
                            <p><strong>Suggested Video Link:</strong> {(viewingSubmission.data as EditEntrySuggestionData).changes.videoLink}</p>
                        </div>
                      )}
                      {(viewingSubmission.data as EditEntrySuggestionData).changes.tags !== undefined && (
                         <div>
                            <p><strong>Current Tags:</strong> {(originalEntryForEditView as ExiconEntry)?.tags?.map(t => t.name).join(', ') || 'None'}</p>
                            <p><strong>Suggested Tags:</strong> {(viewingSubmission.data as EditEntrySuggestionData).changes.tags?.join(', ')}</p>
                        </div>
                      )}
                    </>
                  )}
                  {(viewingSubmission.data as EditEntrySuggestionData).comments && (
                     <div>
                        <p><strong>Submitter Comments:</strong></p>
                        <Textarea value={(viewingSubmission.data as EditEntrySuggestionData).comments} readOnly rows={3} />
                    </div>
                  )}
                </>
              )}
               {viewingSubmission.submissionType === 'edit' && !originalEntryForEditView && !isLoadingOriginalEntry && (
                <p className="text-destructive">Could not load original entry to compare changes.</p>
               )}
            </div>
          )}
          <DialogFooter className="sm:justify-between gap-2 pt-2">
            <div className="flex gap-2">
                <Button 
                    onClick={() => viewingSubmission && handleApproveSubmission(viewingSubmission.id)} 
                ...
                <Button 
                    onClick={() => viewingSubmission && handleRejectSubmission(viewingSubmission.id)} 
                    variant="destructive"
                    disabled={!viewingSubmission}
                >
